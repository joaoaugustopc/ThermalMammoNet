...
def train_model_cv(model, raw_root, message, angle="Frontal", k=5, 
    ...
    for fold, (tr_idx, va_idx, te_idx) in enumerate(
        ...
        def run_fold():
            ...
            if model == "yolo":
                save_split_to_png(X_tr, y_tr, "train", root=f"Yolos_Cls_SegPAD/dataset_fold_{fold+1}")                     #AQUI
                save_split_to_png(X_val, y_val, "val",   root=f"Yolos_Cls_SegPAD/dataset_fold_{fold+1}")                   #AQUI
                save_split_to_png(X_test, y_test, "test", root=f"Yolos_Cls_SegPAD/dataset_fold_{fold+1}")                  #AQUI
                ...
                # model_f.train(
                #     data=f"Yolos_Cls_SegPAD/dataset_fold_{fold+1}",                                    #AQUI
                #     epochs=100,
                #     patience=100,
                #     batch=8,
                #     #lr0=0.0005,
                ...
                #     deterministic=True,
                #     seed=seed,
                #     project="runs/classify/Yolos_Cls_SegPAD",                                         #AQUI 
                #     name=f"YOLOv8_cls_fold_{fold+1}_seed_{seed}"
                # )
                
                # end_time = time.time()

                # best_model_path = f"runs/classify/Yolos_Cls_SegPAD/YOLOv8_cls_fold_{fold+1}_seed_{seed}/weights/best.pt"       #Aqui
                ...
                #     data=f"Yolos_Cls_SegPAD/dataset_fold_{fold+1}",                                                #AQUI
                #     split="test", 
                #     project="runs/classify/val/Yolos_Cls_SegPAD",                       #AQUI
...
def png_para_txt(arquivo_png, arquivo_txt):
    # Carregar a imagem
    img = Image.open(arquivo_png)
    img_array = np.array(img)
    
    # Se for imagem colorida (3 canais), converter para escala de cinza
    if len(img_array.shape) == 3:
        # Converter RGB para escala de cinza usando a fórmula padrão
        img_array = np.dot(img_array[...,:3], [0.2989, 0.5870, 0.1140])
    
    # Salvar no arquivo txt
    np.savetxt(arquivo_txt, img_array, fmt='%.6f')
    
    print(f"Imagem {arquivo_png} convertida para {arquivo_txt}")
    print(f"Dimensões: {img_array.shape}")
    print(f"Valores: min={img_array.min():.3f}, max={img_array.max():.3f}")

def process_heatmaps_for_dataset2(input_base_dir, model_path, output_base_dir):
    """
    Processa todas as imagens nas pastas 0 e 1 do dataset, gerando heatmaps
    
    Args:
        input_base_dir (str): Caminho para o diretório base do dataset (ThermalMammoNet/Yolos_Cls/dataset_fold_1/test)
        model_path (str): Caminho para o modelo .pt
        output_base_dir (str): Diretório base onde os heatmaps serão salvos
    """
    # Cria a estrutura de diretórios de saída
    os.makedirs(os.path.join(output_base_dir, '0'), exist_ok=True)
    os.makedirs(os.path.join(output_base_dir, '1'), exist_ok=True)
    
    # Processa cada classe (0 e 1)
    for class_dir in ['0', '1']:
        input_class_dir = os.path.join(input_base_dir, class_dir)
        output_class_dir = os.path.join(output_base_dir, class_dir)
        
        print(f"\nProcessando classe {class_dir}...")
        print(f"Origem: {input_class_dir}")
        print(f"Destino: {output_class_dir}")
        
        # Processa cada imagem no diretório da classe
        for img_name in os.listdir(input_class_dir):
            if img_name.lower().endswith(('.png', '.jpg', '.jpeg')):  # Filtra apenas imagens
                input_path = os.path.join(input_class_dir, img_name)
                
                # Gera o heatmap
                result_path = heatmap_yolo(
                ...
                if result_path:
                    print(f"✓ Heatmap gerado: {os.path.basename(result_path)}")
                else:
                    print(f"✗ Falha no arquivo: {img_name}")

def heatmap_yolo2(input_image_path, model_path, output_dir):
    """
    Gera um heatmap para uma imagem usando YOLO-CAM e salva o resultado.
    
    Args:
        input_image_path (str): Caminho completo para a imagem de entrada
        model_path (str): Caminho completo para o modelo .pt
        output_dir (str): Diretório onde a imagem com heatmap será salva
    ...
    try:
        ...
        if not all([
            os.path.exists(input_image_path),
            os.path.exists(model_path),
            os.path.exists(output_dir)
        ]):
            raise FileNotFoundError("Um ou mais caminhos não existem")
        
        # Carrega e pré-processa a imagem
        img = cv2.imread(input_image_path)
        if img is None:
            raise ValueError(f"Não foi possível carregar a imagem: {input_image_path}")
        ...
        # Define o nome do arquivo de saída (mesmo nome da entrada + _heatmap)
        input_filename = os.path.splitext(os.path.basename(input_image_path))[0]
        output_filename = f"{input_filename}_heatmap.png"
...
def heatmap_yolo(input_image_path, model_path, output_dir, true_class):
    try:
        if not all([
            os.path.exists(input_image_path),
            os.path.exists(model_path),
        ...
        # Nome do arquivo com status
        input_filename = os.path.splitext(os.path.basename(input_image_path))[0]
        status = "acerto" if correct else "erro"
        output_filename = f"{input_filename}_heatmap_{status}.png"
        output_path = os.path.join(output_dir, output_filename)
        
        cv2.imwrite(output_path, cam_image)

        return output_path, pred_class, confidence, correct
    
    except Exception as e:
        print(f"Erro ao gerar heatmap: {str(e)}")
        return None, None, None, None

def process_heatmaps_for_dataset(input_base_dir, model_path, output_base_dir):
    """
    Processa todas as imagens nas pastas 0 e 1 do dataset, gerando heatmaps
    ...
    for class_dir in ['0', '1']:
        ...
        for img_name in os.listdir(input_class_dir):
            if img_name.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                result_path, pred_class, conf, correct = heatmap_yolo(
                    input_image_path=input_path,
                    model_path=model_path,
                    output_dir=output_class_dir,
                    true_class=int(class_dir)
                )

                if result_path:
                    print(f"✓ Heatmap gerado: {os.path.basename(result_path)} "
                        f"(true={class_dir}, pred={pred_class}, conf={conf:.2f}, {'OK' if correct else 'ERRO'})")
...
def transform_temp_img(input_folder, output_folder):
    ...
    for fname in os.listdir(input_folder):
        if fname.endswith(".txt"):
            ...
            # Primeiro tenta com delimitador padrão (espaço)
            try:
                temperatura = np.loadtxt(path)
            except ValueError:
                # Se falhar, tenta com ponto e vírgula
                try:
                    temperatura = np.loadtxt(path, delimiter=';')
                except ValueError:
                    # Se ainda falhar, tenta com vírgula (para números com vírgula decimal)
                    try:
                        temperatura = np.loadtxt(path, delimiter=',')
                    except ValueError as e:
...
if __name__ == "__main__":
    ...
    #                 seed= SEMENTE,
    #                 segmenter="yolo",
    #                 message="Yolos_Cls_SegPAD",
    #                 resize_method="BlackPadding",
    #                 seg_model_path="runs/segment/train36/weights/best.pt")
    
 
    process_heatmaps_for_dataset("Yolos_Cls_metade_marcador/dataset_fold_1/test", "runs/classify/Yolos_Cls_metade_marcador/YOLOv8_cls_fold_1_seed_13388/weights/best.pt", "heatmaps/Yolos_Cls_metade_marcador/dataset_fold_1")
    process_heatmaps_for_dataset("Yolos_Cls_metade_marcador/dataset_fold_2/test", "runs/classify/Yolos_Cls_metade_marcador/YOLOv8_cls_fold_2_seed_13388/weights/best.pt", "heatmaps/Yolos_Cls_metade_marcador/dataset_fold_2")
    process_heatmaps_for_dataset("Yolos_Cls_metade_marcador/dataset_fold_3/test", "runs/classify/Yolos_Cls_metade_marcador/YOLOv8_cls_fold_3_seed_13388/weights/best.pt", "heatmaps/Yolos_Cls_metade_marcador/dataset_fold_3")
    process_heatmaps_for_dataset("Yolos_Cls_metade_marcador/dataset_fold_4/test", "runs/classify/Yolos_Cls_metade_marcador/YOLOv8_cls_fold_4_seed_13388/weights/best.pt", "heatmaps/Yolos_Cls_metade_marcador/dataset_fold_4")
    process_heatmaps_for_dataset("Yolos_Cls_metade_marcador/dataset_fold_5/test", "runs/classify/Yolos_Cls_metade_marcador/YOLOv8_cls_fold_5_seed_13388/weights/best.pt", "heatmaps/Yolos_Cls_metade_marcador/dataset_fold_5")
